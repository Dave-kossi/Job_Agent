...
CITIES = [
    ...
    "Strasbourg, France",     # Alsace / Grand Est
    "Mulhouse, France",       # Alsace / Grand Est
    "Marseille, France",
    "Montpellier, France",
    "Rennes, France",
    "Nice, France",
...
def collecter_offres(recherche=None, localisation=None, limites=5) -> pd.DataFrame:
    ...
    # Si aucun paramètre n'est passé, on utilise nos listes complètes
    termes_a_chercher = [recherche] if recherche else SEARCH_TERMS[:4] # Ajuste le slice selon le besoin
    ...
    for term in termes_a_chercher:
        for city in villes_a_chercher:
            ...
            try:
                jobs = scrape_jobs(
                    site_name=["linkedin", "indeed", "google"], # Plateformes étendues
                    search_term=term,
                    location=city,
            ...
            time.sleep(1) # Pause de sécurité pour éviter les rate-limits
    ...
    if toutes_les_offres:
        ...
        # Supprime les lignes sans URL et élimine les doublons
